...
def run_gradcam_onfile(model, image_name, target_label):
    ...
    heatmap_layer =  model.layer3[1].conv2
    image = Image.open(image_name).convert('RGB')
    image = image.resize((32,32))
    input_tensor = transform(image)
    gpu_input_tensor = input_tensor.to(device='cuda')
    image = grad_cam(model, gpu_input_tensor, heatmap_layer, target_label)
    return image, target_label

def run_gradcam(model, input_tensor, target_label):
    # model = torchvision.models.resnet34(pretrained=True)
    # model.eval()
    transform = transforms.Compose([
        # transforms.Resize(32,32),
